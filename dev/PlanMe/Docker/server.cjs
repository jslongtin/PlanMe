// code ref / inspired from https://www.youtube.com/watch?v=7nafaH9SddU
...
app.get('/api/allusers/:id', async (req, res) =>
  {
    try {
      const getUsersQuery = 'SELECT * FROM utilisateurs WHERE id = $1';
      const { rows } = await pool.query(getUsersQuery, [req.params.id]);
      res.status(200).json(rows);
      console.log('All users:', rows);
    } catch (err) {
      console.log('Error during getting all users:', err.message);
